...
[![Latest release](https://img.shields.io/github/v/release/rbmrs/fitbyte?include_prereleases&label=download)](https://github.com/rbmrs/fitbyte/releases)
...
The easiest way to try the macOS app is from the [Releases page](https://github.com/rbmrs/fitbyte/releases). Download the latest `Fitbyte-*-macos.zip`, unzip it, then right-click `Fitbyte.app` and choose **Open** on first launch.
...
```bash
fitbyte              # launch the TUI (bare invocation)
...
| `--probe-json` / `--preview-json` / `--progress-json` | — | Structured output for callers driving Fitbyte from another program |
...
A native macOS shell lives in `macos/`. Prebuilt beta builds are on the [Releases page](https://github.com/rbmrs/fitbyte/releases): qualifying pushes to `main` ship a new `Fitbyte-<version>-macos.zip`. See [RELEASES.md](RELEASES.md) for the release flow.
...
Set `FITBYTE_BACKEND=/path/to/app.py` if launching outside the repo.
